...
public class CharacterStats : MonoBehaviour
{
    ...
    private void AttemptyToApplyAilements(CharacterStats _targetStats, int _fireDamage, int _iceDamage, int _lightingDamage)
    {
        ...
        {
            if (Random.value < .34f && _fireDamage > 0)
            {
                canApplyIgnite = true;
    ...
    {
        if (currentHealth > GetMaxHealthValue())
            currentHealth = GetMaxHealthValue();//TODO
    ...
    protected int CheckTargetArmor(CharacterStats _targetStats, int totalDamage)
    {
        if (_targetStats.isChilled)
            ...
            //totalDamage = totalDamage * _targetStats.armor.GetValue()/100+ _targetStats.armor.GetValue();
