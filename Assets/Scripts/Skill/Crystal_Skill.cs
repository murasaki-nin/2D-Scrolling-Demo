...
public class Crystal_Skill : Skill
{
    ...
    public override void UseSkill()
    {
        ...
        else
        {
            ...
            if (cloneInsteadOfCrystal)
            {
                ...
                player.transform.position = new Vector2(currentCrystal.transform.position.x, currentCrystal.transform.position.y - 1);
                currentCrystal.transform.position = playerPos;
            }
            else
            {
                Vector2 playerPos = new Vector2(player.transform.position.x, player.transform.position.y + 1);//player.transform.position;
                player.transform.position = new Vector2(currentCrystal.transform.position.x, currentCrystal.transform.position.y - 1);
                currentCrystal.transform.position = playerPos;
            }
    ...
    {
        currentCrystal = Instantiate(crystalPrefab, new Vector2(player.transform.position.x, player.transform.position.y + 1), Quaternion.identity);
    ...
    private bool CanUseMultiCrystal()
    {
        if (canUseMultiStacks)
        {
            if (crystalLeft.Count > 0)
            {
                ...
                GameObject newCrystal = Instantiate(crystalToSpawn, new Vector2(player.transform.position.x, player.transform.position.y + 1), Quaternion.identity);
